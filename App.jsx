...
function ProjectPage({ project, onBack, onSelectBoard }) {
  ...
  const create = async () => {
    setSaving(true);
    try {
      await api("POST", "/boards", { name: boardName, projectId: project.id }, token);
      toast("Quadro criado!");
      setShowModal(false);
      setBoardName("");
      load();
...
function BoardPage({ board: initialBoard, project, onBack }) {
  ...
  const rows = hasSwimlanes
    ? [
        ...
        ...(cardsWithoutSwimlane || true ? [{ id: null, name: "Sem raia", isSwimlane: false }] : [])
      ]
    : [];
